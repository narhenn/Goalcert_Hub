...
const TABS = [
  { id: 'drill', label: 'Guided Drill', icon: 'ti-school' },
...
function Workspace() {
  ...
  const { graph, engineUp, meta } = useSim()
    ...
    <div className="panel">
      <div className="panel-header">
        ...
        <div className="panel-actions">
          {graph && (
            <span className="pill pill-purple mono" title="Deterministic run id">
              run {graph.rootRunId.slice(0, 8)}
            </span>
          )}
          <span className={`pill ${engineUp === false ? 'pill-red' : engineUp ? 'pill-green' : 'pill-surface'}`}>
            <span className="status-dot" style={{ marginRight: 5 }} />
            {engineUp === false ? 'engine offline' : engineUp ? 'engine live' : 'connecting…'}
          </span>
        </div>
      </div>
        ...
        {tab === 'drill' && (
          active
            ...
            : <div className="empty">
                ...
                <div style={{ marginTop: 6 }}>Open a twin from <b>Twins</b> to start a drill —
                  the simulation tabs below don't need one.</div>
              </div>
        )}
