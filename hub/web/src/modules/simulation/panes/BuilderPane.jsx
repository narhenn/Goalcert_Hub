...
export default function BuilderPane({ onRan }) {
  ...
  if (engineUp === false) {
    return (
      <div className="empty">
        <Icon n="ti-plug-connected-x" /> Simulation engine unreachable.<br />
        Start it with <span className="mono">uvicorn app.main:app --port 8002</span> in
        <span className="mono"> services/simulation-engine/backend</span>, and make sure the hub runs with
        <span className="mono"> SCENARIO_PATH_PREFIX=""</span>.
      </div>
    )
  }
